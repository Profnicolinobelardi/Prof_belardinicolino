...
file_path = 'c:\\Users\\nicol\\OneDrive\\Desktop\\siti internet didattica\\Matematica\\Equazioni esponenziali\\preparazione_verifica.html'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Fix the katex script tag
broken_script_start = '<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.js">'
if broken_script_start in content:
    start_idx = content.find(broken_script_start)
    end_idx = content.find('    });</script>', start_idx)
    
    if end_idx != -1:
        end_idx += len('    });</script>')
        clean_katex = '<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.js"></script>'
        content = content[:start_idx] + clean_katex + content[end_idx:]

# 2. Prepare the correct JS payload
js_payload = r'''
    ...
    function initLab() {
      ...
      for(let i=0; i<eserciziLab.length; i++) {
        ...
        d.onclick = () => { if(elSolBox.style.display==="block" || i < indiceCorrente) { cambiaEsercizioDiretto(i); } };
    ...
    function caricaEsercizio(indice) {
      ...
      // Render math, adding a small timeout to let DOM update
      setTimeout(() => renderMath(document.getElementById("app-container")), 50);
    }
...
# 3. Insert the script into the correct script tag at the end
if 'function openTab(tabId, btn)' in content:
    content = content.replace('function openTab(tabId, btn)', js_payload + '\n    function openTab(tabId, btn)')
