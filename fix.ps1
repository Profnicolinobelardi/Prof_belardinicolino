...
$badScriptStart = '<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.js">'
$idxStart = $content.IndexOf($badScriptStart)
if ($idxStart -ne -1) {
    $idxEnd = $content.IndexOf('    });</script>', $idxStart)
    if ($idxEnd -ne -1) {
        $idxEnd += 14
        $cleanKatex = '<script defer src="https://cdn.jsdelivr.net/npm/katex@0.16.8/dist/katex.min.js"></script>'
        $content = $content.Remove($idxStart, $idxEnd - $idxStart).Insert($idxStart, $cleanKatex)
    }
}
...
if ($content.Contains('function openTab(tabId, btn)')) {
    $content = $content.Replace('function openTab(tabId, btn)', "$jsPayload`n    function openTab(tabId, btn)")
}
